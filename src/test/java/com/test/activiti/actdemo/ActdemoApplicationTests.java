...
import com.test.activiti.actdemo.entity.Person;
...
public class ActdemoApplicationTests {
    ...
    public  void deployPro(){
        ...
        Deployment deployment = repositoryService.createDeployment()
                .addClasspathResource("processes/monAudit.bpmn")
                .addClasspathResource("processes/monAudit.png").deploy();
...
//        String processId = pi.getId();
    ...
    public void viewPic() throws Exception{
        ...
        List<String> list = processEngine
                ...
                .getDeploymentResourceNames("95001");
        String resource="";
        ...
        InputStream stream = processEngine.getRepositoryService()
                .getResourceAsStream("95001", resource);
    ...
    public void startPro() {
        activityDemoServiceImpl.startProcesses("这是业务表的ID version v4");
    }

    //获取受理员任务列表
    @Test
    public void findTasksForSL() {
        List<Task> lists = activityDemoServiceImpl.findTasksByUserId("sQ");
    ...
    public void  climMyTask(){
        String assgin="sQ";
        processEngine.getTaskService().claim("2507",null);
    ...
    public void addCommentMsg(){
        ...
        taskService.addComment("2507",null,"审批通过");
        System.out.println("添加批注成功");
    ...
    public void completeTasksForSL() {
        activityDemoServiceImpl.completeTask("2507", "sQ", "true");//受理后，任务列表数据减少
    }
...
//         1) 获取流程实例的ID
        Task task = this.taskService.createTaskQuery().taskId("12508").singleResult();
...
//       5）返回
    ...
    public void findTasksForSP() {
        List<Task> lists = activityDemoServiceImpl.findTasksByUserId("张三");
    ...
    public void completeTasksForSP() {
        activityDemoServiceImpl.completeTask("80010", "李四", "true");//审批后，任务列表数据减少
    }
...
//        Map<String, Object> map = runtimeService.getVariables("2507");
//        for (Map.Entry<String, Object> entry : map.entrySet()) {
//            System.out.println(entry.getKey() + " : " + entry.getValue());
//        }

        map=taskService.getVariables("2507");
        for (Map.Entry<String, Object> entry : map.entrySet()) {
            System.out.println(entry.getKey() + " : " + entry.getValue());
        }
    }
    ...
    public void setTasksVar() {
        ...
        for(Task task : lists) {//不知为何，变量保存成功，但数据表只有请假天数含有任务id，单获取流程变量时，根据任务id均可获取到（如下一测试）
            taskService.setVariable(task.getId(), "reason", "++审核++");
...
//        }
    ...
    public void queryProImg() throws Exception {
        activityDemoServiceImpl.queryProImg("97501");
    }

    //生成流程图（高亮）---232501
    @Test
    public void queryProHighLighted() throws Exception {
        activityDemoServiceImpl.queryProHighLighted("97501");
    }
